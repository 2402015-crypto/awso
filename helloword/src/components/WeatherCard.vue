...
async function cargarClima() {
  store.cargando = true;
  store.limpiarErrores(); // limpiar errores previos
  try {
    const data = await obtenerClima(store.latitud, store.longitud)
    store.setClima(data.temperatura, data.viento, data.codigoClima)
  } catch (error) {
    store.setError(error.message || 'No se pudo obtener el clima. Intenta nuevamente.')
  } finally {
    store.cargando = false;
  }
}

watch([() => store.latitud, () => store.longitud], () => cargarClima())
...
<template>
  <div class="card">
    ...
    <!-- Bloque de errores -->
    <div v-else-if="store.errores.length > 0" class="error">
